...
## orders.json 의 needsReview / reviewReason / source — 확인필요 배지

`sync_realtime_data.py`(선택 기능)가 추가하는 발주 건에만 붙는 필드입니다.
사람이 직접 적은 발주(`orders.json` 을 손으로 편집했거나 `sync_orders.py` 로
넣은 건)에는 없어도 됩니다 — 없으면 배지도 안 붙습니다.
